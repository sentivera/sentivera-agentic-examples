...
MAX_FILES_TO_PROCESS  = 1000      # default number of files to process
...
5. For each feature, include the exact text from the handbook that supports it.
...
- F-1 <concise, testable feature or task>
  Source: <exact text from handbook>
...
* Always include the exact source text for each feature.
...
def parse_model_output(markdown: str, source_file: str):
    """
    Extract rows:
      {'User Story Number', 'User Story Description', 'Feature Number', 'Feature Description', 'Source File', 'Source Text'}
    ...
    for line in markdown.splitlines():
        ...
        # source text for feature
        if line.lower().startswith("source:") and current_feature:
            current_source = re.sub(r"^source:\s*", "", line, flags=re.I)
            rows.append({
                "User Story Number": current_us_num,
                "User Story Description": current_us_desc,
                "Feature Number": current_feature,
                "Feature Description": current_feature,
                "Source File": source_file,
                "Source Text": current_source
            })
            current_feature = None
            current_source = None
